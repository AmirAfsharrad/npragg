...
M_step = 2
eps_N = 1
MIxz = np.zeros((nIter, eps_N))
MIx = np.zeros((nIter, eps_N))
...
for epsilon_index in range(eps_N):
    ...
    for counter in range(nIter):
        ...
        print("iteration", counter, "out of", nIter)
        for i in range(N):
            # if i % 1000 == 0:
            #     print("N", i / 1000)
            ...
            M1 = np.random.randint(0, L - 1)
            M2 = np.random.randint(0, L - 1)

            theta_hat1 = train_for_theta_hat(M1, L, theta, 0)
            theta_hat2 = train_for_theta_hat(M2, L, theta, 0)
            # print("hello")
        ...
        MIx[counter, epsilon_index] = ss2
        MIxz[counter, epsilon_index] = ss
...
for epsilon_index in range(eps_N):
    ...
    plt.hist(MIxz[:, epsilon_index])
plt.grid(which='both')
plt.legend()
plt.xlabel("$M$")
plt.ylabel("$I(Y_1,Y_2|X,Z)$")
plt.title("Mutual Information of $Y_1, Y_2$ Distribution")
plt.show()
